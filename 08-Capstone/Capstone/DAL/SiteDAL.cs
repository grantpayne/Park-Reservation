...
namespace Capstone.DAL
{
    public class SiteDAL
    {
        ...
        private const string SQL_GetUnreservedCampsites = @"SELECT TOP 5 * FROM site WHERE site.site_id IN (SELECT DISTINCT site.site_id FROM site LEFT JOIN reservation ON reservation.site_id = site.site_id WHERE reservation_id IS NULL OR (site.campground_id = @campgroundID AND NOT ((@reqFromDate <= reservation.to_date AND @reqToDate >= reservation.from_date) OR (reservation.from_date <= @reqFromDate AND reservation.to_date >= @reqToDate))))";
